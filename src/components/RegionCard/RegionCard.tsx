...
const RegionCard = ({ region }: Props) => {
  const { selectedRegionsMetadata, addRegionMetadata, removeRegionMetadata } = useContext(GlobalContext);
  const { t } = useTranslation();

  if (!selectedRegionsMetadata || !addRegionMetadata || !removeRegionMetadata) return null;
    ...
    return (
      <div className={`${className}__data-container`}>
        {labelValMap.map((data: ILabelValMap, key: number) => {
          const countClassName = [`${className}__box-count-value`, `${className}__box-count-value--${data.type}`].join(
            ' '
          );
